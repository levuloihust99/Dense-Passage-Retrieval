...
from libs.data_helpers.data_utils import (
    tokenize_qa, tokenize_context,
    tokenize_qa_with_hardneg
)
from libs.data_helpers.tfio.feature_utils import (
    create_byte_feature,
    create_int_feature
)
...
def serialize_qa_pairs_with_hardneg(
    qa_pair: Dict[Text, List]
):
    features = {}
    for key, value in qa_pair.items():
...
def dump_qa_with_hardneg(
    ...
    example_writer = tf.io.TFRecordWriter(os.path.join(tfrecord_dir, 'data_{:03d}.tfrecord'.format(idx)))
    for pair in query_context_pairs:
        pair_tokenized = tokenize_qa_with_hardneg(
            qa_pair=pair,
            tokenizer=tokenizer,
            query_max_seq_length=query_max_seq_length,
            context_max_seq_length=context_max_seq_length
        )
        tf_example_serialized = serialize_qa_pairs_with_hardneg(pair_tokenized)
