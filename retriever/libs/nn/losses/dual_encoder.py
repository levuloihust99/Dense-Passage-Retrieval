...
class InBatchDotProductLoss(LossCalculator):
    def compute(self, query_embedding: tf.Tensor, context_embedding: tf.Tensor):
...
class InBatchCosineLoss(LossCalculator):
    def compute(self, query_embedding: tf.Tensor, context_embedding: tf.Tensor):
        batch_size, hidden_size = query_embedding.shape.as_list()
        # normalize `query_embedding` and `context_embedding`
        query_embedding = query_embedding / tf.norm(query_embedding, axis=-1, keepdims=True)
        context_embedding = context_embedding / tf.norm(context_embedding, axis=-1, keepdims=True)
        similarity_matrix = tf.matmul(query_embedding, context_embedding, transpose_b=True)
        logits = tf.nn.log_softmax(similarity_matrix, axis=-1)
        loss = tf.gather_nd(
            logits, tf.where(tf.eye(batch_size, dtype=tf.bool))
...
class StratifiedDotProductLoss(object):
    def compute(self, query_embedding: tf.Tensor, context_embedding: tf.Tensor):
        batch_size, hidden_size = query_embedding.shape.as_list()
...
class StratifiedCosineLoss(object):
    def compute(self, query_embedding: tf.Tensor, context_embedding: tf.Tensor):
        batch_size, hidden_size = query_embedding.shape.as_list()
        within_size = context_embedding.shape[0] // batch_size
        # normalize `query_embedding` and `context_embedding`
