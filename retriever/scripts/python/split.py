...
from libs.data_helpers.split import train_test_split
...
def split_data(all_data, train_ratio: float = 0.8):
    all_items = all_data.get('items')
    L = len(all_items)
    train_indices, test_indices = train_test_split(L, train_ratio)
    ...
    return {'_name_': 'train', '_count_': len(train_items), 'items': train_items}, \
        {'name': '_test_', '_count_': len(
            test_items), 'items': test_items}, train_indices, test_indices
...
def main():
    ...
    parser.add_argument("--out-train-path",
                        default="data/json/train/data.json")
    ...
    logger.info(
        "Done loading question-context pairs in {}s".format(time.perf_counter() - start_time))

    logger.info("Splitting train/test data...")
    start_time = time.perf_counter()
    train_data, test_data, train_indices, test_indices = train_test_split(
        all_data, args.train_ratio)
    logger.info(
        "Done splitting train/test data in {}s".format(time.perf_counter() - start_time))
