...
def create_query_dataset(
    ...
    logger.info("Done creating query dataset in {}s".format(time.perf_counter() - start_time))
...
def generate_query_embeddings(
    ...
    for batch in query_dataset:
        outputs = query_encoder(input_ids=batch['input_ids'], attention_mask=batch['attention_mask'], return_dict=True, training=False)
        batch_embeddings = outputs.last_hidden_state[:, 0, :]
        query_embeddings.extend(batch_embeddings.numpy())
    logger.info("Done generating query embeddings in {}s".format(time.perf_counter() - start_time))
...
def evaluate(
    ...
    for batch_idx in range(0, len(query_embeddings), num_queries_per_search):
        query_vectors = query_embeddings[batch_idx : batch_idx + num_queries_per_search]
